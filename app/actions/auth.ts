		...
		for (const txn of userTransactions) {
			await tx
				...
				.set({
					...
					note: txn.note ? await encrypt(txn.note, encryptionKey) : null,
				})
		...
		for (const debt of userDebts) {
			await tx
				...
				.set({
					...
					remainingAmount: await encrypt(debt.remainingAmount, encryptionKey),
	...
	await db.transaction(async (tx) => {
		for (const acc of userAccounts) {
			await tx
				...
				.set({
					...
					balance: await encrypt(await decrypt(acc.balance, oldKey), newKey),
				})
				.where(eq(accounts.id, acc.id));
		}
		...
		for (const txn of userTransactions) {
			await tx
				.update(transactions)
				.set({
					amount: await encrypt(await decrypt(txn.amount, oldKey), newKey),
		...
		for (const budget of userBudgets) {
			await tx
				.update(budgets)
				.set({
					amount: await encrypt(await decrypt(budget.amount, oldKey), newKey),
				})
				.where(eq(budgets.id, budget.id));
		}
		...
		for (const rec of userRecurring) {
			await tx
				.update(recurringTransactions)
				.set({
					amount: await encrypt(await decrypt(rec.amount, oldKey), newKey),
					description: await encrypt(
		...
		for (const debt of userDebts) {
			await tx
				...
				.set({
					...
					amount: await encrypt(await decrypt(debt.amount, oldKey), newKey),
					remainingAmount: await encrypt(
						await decrypt(debt.remainingAmount, oldKey),
						newKey,
					),
					description: debt.description
						? await encrypt(await decrypt(debt.description, oldKey), newKey)
						: null,
				})
				.where(eq(debts.id, debt.id));
		}
