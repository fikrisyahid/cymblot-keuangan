...
function buildNetFlow(
	...
	if (period === "yearly") {
		...
		for (const txn of txns) {
			...
			map.set(
				m,
				(map.get(m) || 0) + (txn.type === "INCOME" ? amount : -amount),
			);
		}
	} else if (period === "monthly") {
		const daysInMonth = new Date(
		...
		for (const txn of txns) {
			...
			map.set(
				d,
				(map.get(d) || 0) + (txn.type === "INCOME" ? amount : -amount),
			);
		}
	} else {
		const dayNames = ["Sen", "Sel", "Rab", "Kam", "Jum", "Sab", "Min"];
		...
		for (let i = 0; i < 7; i++) {
			map.set(
				`${dayNames[i]} ${current.getDate()}/${current.getMonth() + 1}`,
				0,
			);
			current.setDate(current.getDate() + 1);
		}
		for (const txn of txns) {
			...
			map.set(
				key,
				(map.get(key) || 0) + (txn.type === "INCOME" ? amount : -amount),
			);
		}
	}
