...
export async function getTransfers(filters?: TransferFilters) {
	...
	if (filters?.startDate)
		conditions.push(gte(transfers.date, filters.startDate));
...
export async function getTransfersPaginated(
	...
	if ((filters?.search && key) || filters?.accountId) {
		...
		let allDecrypted: TransferRow[] = key
			? await Promise.all(
					allRaw.map((t) => decryptTransfer(t as TransferRow, key)),
				)
	...
	const data: TransferRow[] = key
		? await Promise.all(
				rawData.map((t) => decryptTransfer(t as TransferRow, key)),
			)
...
export async function updateTransfer(
	...
	try {
		...
		await db
			...
			.where(and(eq(transfers.id, id), eq(transfers.userId, session.userId)));
...
export async function deleteTransfer(id: string): Promise<ActionResult> {
	...
	try {
		...
		// Decrypt amounts
		const amountStr = key
			? await decrypt(transfer.amount, key)
			: transfer.amount;
		const feeStr =
			key && transfer.fee
				? await decrypt(transfer.fee, key)
				: transfer.fee || "0";
