  ...
  return (
    <Stack gap="lg">
      ...
      <Group justify="space-between" align="flex-start" wrap="wrap">
        <Group gap="sm">
          <ThemeIcon size="lg" radius="xl" variant="gradient" gradient={{ from: "violet", to: "blue", deg: 135 }}>
              ...
              <Paper p="md" radius="md" withBorder h="100%">
                ...
                {data.expenseByCategory.length > 0 ? (
                  <Stack>
                    ...
                    <Divider />
                    <CategoryList items={data.expenseByCategory} total={data.totalExpense} />
                  </Stack>
                ) : (
                  <EmptyState />
                )}
              ...
              <Paper p="md" radius="md" withBorder h="100%">
                ...
                {data.incomeByCategory.length > 0 ? (
                  <Stack>
                    ...
                    <Divider />
                    <CategoryList items={data.incomeByCategory} total={data.totalIncome} />
                  </Stack>
                ) : (
                  <EmptyState />
                )}
                    ...
                    <Paper p="sm" radius="md" bg="var(--mantine-color-default-hover)">
                      ...
                      <Group gap="xs" mb="xs">
                        <Text size="sm" fw={600} c="teal">
                          {data.totalIncome + data.totalExpense > 0
                            ? ((data.totalIncome / (data.totalIncome + data.totalExpense)) * 100).toFixed(1)
                            : 0}
                          %
                        </Text>
                        ...
                        <Text size="sm" fw={600} c="red">
                          {data.totalIncome + data.totalExpense > 0
                            ? ((data.totalExpense / (data.totalIncome + data.totalExpense)) * 100).toFixed(1)
                            : 0}
                          %
                        </Text>
                      </Group>
                      <Progress.Root size="lg" radius="xl">
                        <Progress.Section
                          value={
                            data.totalIncome + data.totalExpense > 0
                              ? (data.totalIncome / (data.totalIncome + data.totalExpense)) * 100
                              : 0
                          }
                          color="teal"
                        >
                        ...
                        <Progress.Section
                          value={
                            data.totalIncome + data.totalExpense > 0
                              ? (data.totalExpense / (data.totalIncome + data.totalExpense)) * 100
                              : 0
                          }
                          color="red"
                        >
...
function CategoryList({
  ...
  items: { name: string; value: number; color: string; icon: string; count: number }[];
  total: number;
}) {
  return (
    <Stack gap="xs">
...
function AccountRow({ account }: { account: { id: string; name: string; type: string; balance: number; income: number; expense: number; color: string } }) {
