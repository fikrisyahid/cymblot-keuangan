...
async function TransactionsContent({
	...
	const txFilters = {
		type: isTransferFilter
			? undefined
			: (searchParams.type as "INCOME" | "EXPENSE" | undefined),
	...
	if (needUnifiedPagination) {
		...
		type MergedItem =
			| {
					...
					date: Date;
					createdAt: Date;
					data: (typeof allTransactions)[0];
			  }
			| {
					kind: "transfer";
					date: Date;
					createdAt: Date;
					data: (typeof allTransfers)[0];
			  };
		...
		return (
			<TransactionsClient
				transactions={paged
					...
					.map((i) => i.data as (typeof allTransactions)[0])}
				transfers={paged
					.filter((i) => i.kind === "transfer")
					.map((i) => i.data as (typeof allTransfers)[0])}
				accounts={accounts}
